...
# Move input of pre-notebook to output directory
def copy_input():
    if not os.path.exists(OUTPUT_DIR):
        os.makedirs(OUTPUT_DIR)
    for filename in os.listdir(INPUT_DIR):
        src = os.path.join(INPUT_DIR, filename)
        if not src.endswith('.ipynb') or not src.endswith('.json'):
...
def training(model, train_loader, val_loader, optimizer, scheduler, scaler, device):
    ...
    if CONTINUOUS:
        ...
        print(f"Resuming training from epoch {start_epoch} with loss {loss} and validation loss {valid_loss}")
        copy_input()
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        save_checkpoint(model_to_log, optimizer, scheduler, epoch + 1, total_loss_epoch / len(train_loader), best_val_loss, patience_counter, filename=f"checkpoint_epoch_{epoch + 1}.pth")
    ...
    save_checkpoint(model_to_log, optimizer, scheduler, EPOCHS, total_loss_epoch / len(train_loader), best_val_loss, patience_counter, filename="checkpoint_final.pth")
